...
mixers = ["mixer %d" % a for a in range(alpha)]
verifier = ['verifier']
mixers_and_verifier = mixers + verifier
...
def mix(ck, ck_fo, permcomm, enc_msgs, enc_msg_shares, enc_rand_shares, alpha, pai_pk, pai_pklist_single, _pai_sklist, _pai_sklist_single, _pi, _svecperm):
    ...
    with timer("re-encryption and permutation", report_subtimers=mixers):
        status_shuffle = True
    ...
    # Decryption
    with timer("decryption of output messages", report_subtimers=mixers):
    ...
    with timer("decryption of individual message/randomness shares", report_subtimers=mixers):
...
def check_verfsigs(msgs_out, sigs, verfpk, enc_sigs, enc_sigs_rands, elgpk):
...
def get_blsigs(enc_sigs, ck, permcomm, alpha, elgpk, _svecperm, _pi, _re_pi, _elgsklist):
    ...
    with timer("re-encrypt and reverse-permute BB signatures", report_subtimers=mixers):
    ...
    with timer("generate encrypted blinded BB signatures", report_subtimers=mixers):
        _blshares = []
        enc_bls, pf = [], []
        for a in range(alpha):
        ...
        enc_blsigs = [elgamal_encrypt(elgpk, g1 ** 0, randIn=0)] * myn
        for a in range(alpha):
    ...
    with timer("decrypt blinded signatures", report_subtimers=mixers):
...
def check_verfsigs_rev(sigs_rev, comms, verfpk, enc_sigs_rev, enc_sigs_rev_rands, elgpk, paipk):
...
def get_blsigs_rev(enc_sigs_rev, enc_rands, ck, ck_fo, permcomm, alpha, auth_elgpk, auth_paipk, _svecperm, _rand_shares, _pi, _auth_elgsklist, _auth_paisklist):
    ...
    # Generate encrypted blinded signatures
    with timer("generate encrypted blinded BBS+ signatures", report_subtimers=mixers):
    ...
    # Decryption of blinded signatures
    with timer("decryption of blinded BBS+ signatures", report_subtimers=mixers):
...
def main(n, alpha):
    ...
    with timer("checking inputs"):
        status_encs = timed(check_encs, report_subtimers=mixers)(pai_pk, pai_pklist_single, enc_msgs, enc_rands, enc_msg_shares, enc_rand_shares, pf_encmsgs, pf_encrands, pfs_enc_msg_shares, pfs_enc_rand_shares)
        pprint("status_encs:", status_encs)

    ####### Mixing process #######

    with timer("mixing", report_subtimers=mixers):
    ...
    with timer("forward set membership", report_subtimers=mixers_and_verifier):
        ...
        status_verfsigs = timed(check_verfsigs, report_subtimers=mixers)(msgs_out, sigs, verfpk, enc_sigs, enc_sigs_rands, elg_pk)
        pprint("status_verfsigs:", status_verfsigs)
        blsigs, _blshares = timed(get_blsigs, report_subtimers=mixers)(enc_sigs, ck, permcomm, alpha, elg_pk, _svecperm, _pi, _re_pi, _elg_sklist)

        # Proofs
        dpk_bbsig_pfs = timed(dpk_bbsig_nizkproofs, report_subtimers=mixers)(comms, blsigs, verfpk, alpha, _msg_shares, _rand_shares, _blshares)
    ...
    with timer("reverse set membership", report_subtimers=mixers_and_verifier):
        ...
        status_verfsigs_rev = timed(check_verfsigs_rev, report_subtimers=mixers)(sigs_rev, comms, verfpk, enc_sigs_rev, enc_sigs_rev_rands, elg_pk, pai_pk)
        pprint("status_verfsigs_rev:", status_verfsigs_rev)
        blsigs_rev, _blshares_rev = timed(get_blsigs_rev, report_subtimers=mixers)(enc_sigs_rev, enc_rands, ck, ck_fo, permcomm, alpha, elg_pk, pai_pk, _svecperm, _rand_shares, _pi, _elg_sklist, _pai_sklist)

        # Proofs
        blsigs_S, blsigs_c, blsigs_r = blsigs_rev
        _blshares_S, _blshares_c, _blshares_r = _blshares_rev
        dpk_bbsplussig_pfs = timed(dpk_bbsplussig_nizkproofs, report_subtimers=mixers)(msgs_out, blsigs_S, blsigs_c, blsigs_r, verfpk, alpha, _blshares_S, _blshares_c, _blshares_r)
